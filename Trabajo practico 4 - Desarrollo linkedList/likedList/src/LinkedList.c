...
{
    int returnAux = -1;
    int lengthis = ll_len(this);
    int lengthis2 = ll_len(this2);
    int i;
    int containIsTrue;
    int countIsTrue = 0;
    //void* pElementThis;
    ...
    {
    	if(lengthis == lengthis2)
    	{
    		for(i = 0; i <= lengthis; i++)
    		{
    			pElementThis2 = ll_get(this2, i);
    			containIsTrue = ll_contains(this, pElementThis2);
    			if(containIsTrue == 1)
    			{
    				countIsTrue++;
    			}
    		}
    		if(countIsTrue == lengthis)
    			returnAux = 1;
...
LinkedList* ll_subList(LinkedList* this,int from,int to)
{
    ...
    if(this != NULL && from > -1 && from < to && to <= len)
    {
    	...
    	if(cloneArray != NULL)
    	{
    		...
    		{
    			pElementAux = ll_get(this,from+i);
    			addNode(cloneArray, i,pElementAux);
    		}
    	}
    }
    return cloneArray;
...
LinkedList* ll_clone(LinkedList* this)
{
    ...
    if (this != NULL)
    {
    	cloneArrayAux = ll_subList(this,0,ll_len(this));
    ...
    if (this != NULL && pFunc != NULL && (order == 0 || order == 1))
    {
    	for(i = 0 ; i < len-1 ; i++)
    	{
    		for (j = i+1; j< len; j++)
    		{
				...
				orderFunc = pFunc(auxElement1,auxElement2);

				if((orderFunc== 1 && order == 1))
				{
					ll_set(this, i,auxElement2);
					ll_set(this, j ,auxElement1);
					returnAux = 0;
				}
				else if((orderFunc== -1 && order == 0))
				{
					ll_set(this, i,auxElement2);
					ll_set(this, j ,auxElement1);
					returnAux = 0;
